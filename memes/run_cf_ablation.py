...
print("Loading best-model checkpoint for intermediate fusion ...")
best_ckpt_bundle = _load_checkpoint_config(BEST_MODEL_PATH)
...
print("Extracting best-model latent representations ...")
_train_latents, _train_pred_labels = _extract_latents_and_preds(
    best_model,
    _make_loader(train_images, train_texts, dataset.y_train, shuffle=False),
)
_test_latents, _ = _extract_latents_and_preds(
    best_model,
    _make_loader(test_images, test_texts, dataset.y_test, shuffle=False),
)
...
def _predict_single_live(text: str, image) -> float:
    if best_cfg["text_encoder"] == "word2vec":
        ...
        encoded = best_text_backend(
            text,
            max_length=best_cfg.get("max_len", 128),
            padding="max_length",
            truncation=True,
            return_tensors="pt",
        )
    ...
    img_tensor = best_image_transform(Image.fromarray(np.asarray(image, dtype=np.uint8))).unsqueeze(0)
