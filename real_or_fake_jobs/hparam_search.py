"""
Hyperparameter search for the multimodal job-posting classifier.
...
Outputs in real_or_fake_jobs/data/
-----------------------------------
...
# CLI args
# ---------------------------------------------------------------------------
parser = argparse.ArgumentParser(description="Hyperparameter search for multimodal job classifier")
...
X_train_description_all = data["X_train_description"].tolist()
...
class JobDataset(Dataset):
    def __init__(self, X_static, X_description, X_profile, X_reqs, y, max_len=512):
    ...
    def __getitem__(self, idx):
        enc_desc    = self._enc(self.X_description[idx])
...
full_train_ds = JobDataset(X_train_static_all,
                           X_train_description_all, X_train_profile_all, X_train_reqs_all,
                           y_train_all)
test_ds       = JobDataset(X_test_static,
                           X_test_description, X_test_profile, X_test_reqs,
                           y_test)
...
def run_epoch(model, loader, optimizer, criterion, train):
    ...
    with torch.set_grad_enabled(train):
        for batch in loader:
            ...
            reqs_ids    = batch["reqs_ids"].to(DEVICE)
            reqs_mask   = batch["reqs_mask"].to(DEVICE)
            tab         = batch["X_static"].to(DEVICE)
            y           = batch["label"].to(DEVICE)
            logits = model(desc_ids, desc_mask, profile_ids, profile_mask,
                           reqs_ids, reqs_mask, tab)
