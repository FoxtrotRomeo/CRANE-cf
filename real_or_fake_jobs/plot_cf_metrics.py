"""
Plot counterfactual metrics for all generators on the real_or_fake_jobs dataset.
...
# Canonical NN generators — drop Text_description (identical to Text) and
# bracket-notation aliases (Text[company_profile], Text[requirements])
NN_GENERATORS = [
    "Tabular",
    "Text",
...
def load_genetic_data(base_dir: Path, variants: list[tuple[str, str]]) -> pd.DataFrame:
    ...
    Returns one row per patient per variant (objectives averaged over Pareto candidates).
    """
    records = []
...
def load_genetic_flip_rate(base_dir: Path, variants: list[tuple[str, str]]) -> pd.DataFrame:
    ...
    for name, subfolder in variants:
        ...
        records.append({"generator": name, "flip_rate": n_flipped / n_total})
    return pd.DataFrame(records)
...
def plot_aggregated(summary_df: pd.DataFrame):
    ...
    fig.suptitle("Counterfactual metrics — real_or_fake_jobs", fontsize=14, fontweight="bold")
...
def plot_flip_rate(flip_df: pd.DataFrame):
    ...
    fig.suptitle("Flip rate ↑ (% CFs achieving target class) — real_or_fake_jobs",
